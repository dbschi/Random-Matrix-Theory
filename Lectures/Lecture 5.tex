...
\theorem[placherelrotach]{Plancherel Rotach}{
    Let $\epsilon >0$. Then \begin{enumerate}
        \item For $x=\sqrt{2N+1} \cos \theta$, $\epsilon \leq \theta \leq \pi - \epsilon$, \begin{align*}
            &e^{-x^2/2}H_N(x) \\
            = &2^{N/2 +1/4} {(N!)}^{1/4} (\pi N)^{-1/4} (\sin \theta)^{-1/2}\Bigg\{\sin\Bigg(\Bigg(\frac{N}{2}+\frac{1}{4}\Bigg)(\sin 2\theta - 2\theta)+\frac{3\pi}{4}\Bigg)\Bigg\} + O(N^{-1}).
        \end{align*}
        
        \item For $x=\sqrt{2N+1} \cosh \theta$, $\epsilon \leq \theta \leq M < \infty$
        \begin{align*}
         &e^{-x^2/2}H_N(x) \\
         =& 2^{N/2-3/4} (N!)^{1/2}(\pi N)^{-1/4}(\sinh \theta)^{-1/2} \exp \Bigg(\Big(\frac{N}{2}+\frac{1}{4}\Big) (2\theta - \sinh 2\theta)\Bigg) (1+O(N^{-1})).
        \end{align*}
        \item For $x=\sqrt{2N+1} - 2^{-1/2}3^{-1/3}N^{-1/6}t$,
        \begin{align*}
         &e^{-x^2/2}H_N(x) \\
         =& 3^{1/3}\pi^{-3/4}2^{N/2+1/4}(N!)^{1/2}N^{-1/12}[A(t)+O(N^{-2/3})],
        \end{align*}
    \end{enumerate}
    where \[
    A(t) = \pi A_i(-3^{-1/3} t),
    \]
    $A_i$ is the Airy function \[
    A_i (t) = \frac{1}{2\pi i}\int_C e^{z/3-tz} dz, 
    \]
    along the curve $C$
...
\begin{tikzpicture}[scale=1.5]
    ...
    \draw[red] (0.289,0.5) arc(60:90:0.577);
    \draw[red] (0.289,-0.5) arc(-60:-90:0.577);
...
\begin{remark}
    This is in Szego's Orthogonal Polynomials, the to-go book for orthogonal polynomials. (This comes up in google if you search for `blue book laplace method asymptotic exansion of integrals'). The proof of these relies on the integral representation and Laplace descent. 

    The Plancherel Rotach method works for other orthogonal polynomials, but the Hermite polynomials for $x^2$ demonstrate enough.
\end{remark}
\begin{remark}
...
Sounds good, but good luck computing this... instead we can compute it in a different way \begin{alemma}{}{}
    Let $A$ be an integral operator with $K$ as its kernel. Then \[
